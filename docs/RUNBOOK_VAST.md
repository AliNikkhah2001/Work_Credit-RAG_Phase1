...
- **HurtLex — fixed via allowlist 2026-09-02 (guardrails `e59b300`):** conservative lexicon flagged `حذف, بخشی, تامین مالی, اشتغال, پست, مصرف, هدف, نادرست` as `hate` (e.g., `چگونه می‌توانم گزارش اعتباری...` was blocked at input as `hate:حذف` because KB context contains `درخواست حذف سابقه منفی قدیمی`, so RAG returned 0 citations). Now `kb/hurtlex_allowlist.json` allowlists these 8 legitimate credit terms (exact-word `\b...\b`, normalized, logged). Profanity/PII/secret remain strict. After fix, 6/6 credit queries return `stop` with 5 citations; genuine hate like `حرامزاده`/`احمق` still blocked.
